...
<script type="text/javascript">
	...
	function updateFormBp(bp_num){
		window.open("admin_bp_update.do?bp_num="+bp_num, "", "width=640, height=400")
	}
	...
	function viewBFeed(bp_num, mem_num){
		window.open("b_personalHome.do?bp_num="+bp_num+"&num="+mem_num, "", "width=1000, height=800, left=680, top=270")
	}
				...
				<div class="row row-cols-2" style="height: 800px; width: 1400px;">
			    	<div class="col" style="overflow: scroll; height: 800px; width: 700px;">
						...
						<table class="table align-middle" width="80%" height="80%">
								...
								<tr align="right">
									...
									<button type="button" class="btn btn-secondary btn-sm" onclick="window.location='admin_member_list.do?view=b'">돌아가기</button>
									</td>	
								</tr>
						</table>
					</div>
					<div class="col" style="width: 500px;">
							...
							<div class="col" style="height: 400px; overflow: scroll;">
								...
								<br>
								<div align="left">
									<h6><strong>프로필 # : </strong>${getBp.bp_num}</h6>
									<h6><strong>회원 # : </strong>${getBp.mem_num}</h6>
									<%-- <h6>
										<c:if test="${not empty getBp.bp_img}"><img src="resources/img/${getBp.bp_img}" width="150" height="150"></c:if>
										<c:if test="${empty getBp.bp_img}"><img src="resources/img/basic_profile.jpg" width="150" height="150"></c:if>
									</h6> --%>
									
									<h6><strong>매장명 : </strong>${getBp.bp_store_name}</h6>
									<h6><strong>연락처 : </strong>${getBp.bp_tel}</h6>
									<h6><strong>별점 : </strong><img src="resources/img/star2.png" width="25" height="25">&nbsp;${getBp.bp_starrating}</h6>
									<h6><strong>주소 : </strong>${getBp.bp_location}</h6>
									<h6><strong>상세주소 : </strong>${getBp.bp_d_location}</h6>
									<h6><strong>정보 : </strong>${getBp.bp_info}</h6>
									<a href="javascript:updateFormBp('${getBp.bp_num}')"><button type="button" class="btn btn-secondary btn-sm">비즈니스 프로필 수정</button></a>
									<a href="javascript:viewBFeed('${getBp.bp_num}','${getBp.mem_num}')"><button type="button" class="btn btn-secondary btn-sm">비즈니스 피드 보기</button></a>
								</div>
							</div>
